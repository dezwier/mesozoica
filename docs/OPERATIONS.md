...
The RAG package configuration and usage are documented in [`../backend/rag/README.md`](../backend/rag/README.md) and [`../backend/rag/docs/USAGE.md`](../backend/rag/docs/USAGE.md), with detailed recovery in [`../backend/rag/docs/OPERATIONS.md`](../backend/rag/docs/OPERATIONS.md). The app runs a single `dinosaur_knowledge` cron that acquires, embeds, and ingests each dinosaur/source independently across `dinosaur_knowledge_source` / `dinosaur_knowledge_doc` / `dinosaur_knowledge_chunk`; failed or interrupted sources remain visible and resume on the next run. Embeddings are persisted on chunk rows before Azure upload so ingest failures do not force re-embedding. Content and pipeline fingerprints both gate currency. Index deletion is never implicit. Use `--recreate-index` only after verifying the configured Azure Search service/index because it causes downtime, deletes and rebuilds that exact index, then marks acquired sources pending for re-ingest from SQL.
...
- Public legal routes `/privacy`, `/terms`, `/delete-account`, and `/delete-data` **301** to `https://learnfromdata.ai/mesozoica/...`. Confirm the Learn from Data pages load before a store submission. See [`store-launch.md`](store-launch.md).
