...
`backend/app/main.py` exports the app built by `app/core/app_factory.py`. The factory registers exception mapping, CORS, game-config version middleware, health routes, public legal redirects (`/privacy`, `/terms`, `/delete-account`, `/delete-data` → `https://learnfromdata.ai/mesozoica/...`), the `/api/v1` router, and static media mounts.
