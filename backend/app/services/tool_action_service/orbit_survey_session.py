"""Orbit Survey sessions: timed period-mosaic overlay."""
...
from app.models.orbit_survey_session import (
...
def _expire_if_needed(session: Session, row: OrbitSurveySession) -> OrbitSurveySession:
...
def get_active_orbit_survey_session(
    session: Session,
    *,
    user_id: int,
) -> OrbitSurveySession | None:
    """Return the user's active (non-expired) orbit survey session, if any."""
    row = session.exec(
        select(OrbitSurveySession)
        .where(
            col(OrbitSurveySession.user_id) == user_id,
            col(OrbitSurveySession.status) == SESSION_STATUS_ACTIVE,
        )
        .order_by(col(OrbitSurveySession.started_at).desc())
    ).first()
    if row is None:
        return None
...
def cancel_active_orbit_survey_sessions(
    session: Session,
    *,
    user_id: int,
) -> None:
...
def start_orbit_survey_session(
    session: Session,
    *,
    user_id: int,
    tool_id: int,
    ...
    if selected is None:
        ...
        raise ValidationError("You must own Orbit Survey to use it")
...
def cancel_orbit_survey_session(
    session: Session,
    *,
    user_id: int,
    ...
    if session_id is not None:
        row = session.get(OrbitSurveySession, session_id)
        if row is None or int(row.user_id) != user_id:
            raise NotFoundError(f"Orbit survey session {session_id} not found")
        row = _expire_if_needed(session, row)
        if row.status == SESSION_STATUS_ACTIVE:
            now = _utcnow()
            row.status = SESSION_STATUS_CANCELLED
    ...
    row = get_active_orbit_survey_session(session, user_id=user_id)
    if row is None:
        return None
    now = _utcnow()
    row.status = SESSION_STATUS_CANCELLED
