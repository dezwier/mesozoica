...
from app.features.field.application.field_fossil_generate import (
...
from app.features.field.application.field_generate import (
    FieldSiteLazyConfig,
...
def process_one_job(*, worker_id: str) -> bool:
    """Process one ensure, survey, or tool-session tick; prefer ensure then survey."""
    if process_one_ensure_job(worker_id=worker_id):
        return True
    if process_one_survey_job(worker_id=worker_id):
        return True
    with Session(engine) as session:
        from app.features.tools.public import process_tool_session_tick

        return process_tool_session_tick(session)
