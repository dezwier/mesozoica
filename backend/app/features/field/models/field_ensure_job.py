"""Queued field-site ensure jobs (one row per spatial cell). Owned by the field feature."""
...
class FieldEnsureJob(SQLModel, table=True):
    ...
    generated_count: Optional[int] = Field(default=None)
    total_in_radius: Optional[int] = Field(default=None)
