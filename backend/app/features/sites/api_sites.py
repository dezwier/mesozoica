"""Site HTTP endpoints owned by the sites feature."""
...
from app.shared.data_sources import DATA_SOURCE_ARCHIVE, DATA_SOURCE_FIELD
...
from app.features.sites.application.identify import (
    get_identify_options,
    submit_identify_guess,
)
from app.features.sites.application.summary import SiteRow
from app.features.progression.public import get_skill_xp, level_for_xp

catalog_router = APIRouter(prefix="/sites", tags=["sites"])
field_router = APIRouter(prefix="/sites", tags=["sites"])
detail_router = APIRouter(prefix="/sites", tags=["sites"])
...
@catalog_router.get("", response_model=SiteListResponse)
def get_sites(
...
@catalog_router.get("/nearby", response_model=SiteNearbyResponse)
...
@catalog_router.get("/nearby-discoverable", response_model=SiteNearbyResponse)
...
@field_router.post("/field/ensure", response_model=FieldEnsureResponse, status_code=202)
...
@field_router.get("/field/ensure/jobs/{job_id}", response_model=FieldEnsureJobResponse)
...
@field_router.delete("/field", response_model=FieldDataPurgeResponse)
...
@field_router.get("/survey/jobs/{job_id}", response_model=FieldSurveyJobResponse)
...
@field_router.post("/{site_id}/discover", response_model=FieldDiscoverResponse)
...
@field_router.post("/{site_id}/status", response_model=SiteSummary | FieldDiscoverResponse)
...
@field_router.post("/{site_id}/discard", status_code=status.HTTP_204_NO_CONTENT)
...
@field_router.get("/{site_id}/identify-options", response_model=IdentifyOptionsResponse)
...
@field_router.post("/{site_id}/identify", response_model=IdentifyGuessResponse)
...
@detail_router.get("/{site_id}", response_model=SiteSummary)
def get_site(
    site_id: int,
    session: Session = Depends(get_session),
    current_user: User | None = Depends(get_optional_current_user),
...
@detail_router.get("/{site_id}/fossils", response_model=SiteFossilThumbListResponse)
...
@detail_router.get("/{site_id}/dinosaurs", response_model=SiteDinosaurThumbListResponse)
...
@detail_router.get("/{site_id}/groups", response_model=SiteDinoFossilGroupListResponse)
...
# Compatibility router consumed by the unchanged v1 composition root. Route
# order is preserved because it is part of the OpenAPI contract snapshot.
router = APIRouter()
