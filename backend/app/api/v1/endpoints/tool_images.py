"""Admin upload endpoint for curated tool card images."""
...
def _require_sync_secret(
    ...
    expected = settings.tool_image_sync_secret.strip()
    if not expected:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Tool image sync is not configured on this server.",
        )
    provided = (x_tool_image_sync_key or "").strip()
    if not provided or not secrets.compare_digest(provided, expected):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid tool image sync key.",
        )
