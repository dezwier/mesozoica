"""Validated settings for chunking, embeddings, and Azure AI Search."""
...
class KnowledgeBaseSettings(BaseSettings):
    """Configuration for chunking, embedding, indexing, and retrieval.

    Azure Search deliberately uses separate credentials: an admin key for schema and
    write operations, and a query key for the least-privileged retrieval client.
    """
    ...
    def validate_pipeline(self) -> "KnowledgeBaseSettings":
        ...
        if problems:
            raise KnowledgeBaseConfigurationError("; ".join(problems))
        return self
