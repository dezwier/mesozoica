...
def test_start_orbit_survey_session_snapshots_and_replaces(
    ...
    first = client.post(
        f"/api/v1/tools/{fmap.id}/actions/orbit-survey-session",
        headers=headers,
    )
    ...
    rows = session.exec(select(OrbitSurveySession)).all()
    assert len(rows) == 2
...
def test_cancel_and_restore_orbit_survey(client, session: Session) -> None:
    ...
    active = client.get(
        "/api/v1/tools/sessions/orbit-survey/active",
        headers=headers,
    )
    ...
    cancelled = client.post(
        "/api/v1/tools/sessions/orbit-survey/cancel",
        headers=headers,
    )
...
def test_expired_orbit_survey_session_ignored(session: Session) -> None:
    ...
    row = session.exec(select(OrbitSurveySession)).one()
