...
void main() {
  ...
  test('loads shared YAML with current game defaults', () async {
    ...
    expect(config.siteStewardship.mainParams.documentationAccuracy, 0.01);
    ...
    final accMods = config.siteStewardship.levelModifiers['documentation_accuracy']!;
    expect(accMods.length, 2);
    expect(accMods.first.level, 1);
    expect(accMods.first.op, 'multiply');
    expect(accMods.first.value, 1);
    expect(accMods.last.level, 99);
    expect(accMods.last.value, 99);
    final rivalMods = config.siteStewardship.levelModifiers['rival_discovery_chance']!;
    expect(rivalMods.length, 2);
    ...
    expect(config.siteStewardship.levelModifiers.containsKey('documentation_genera'), isFalse);
