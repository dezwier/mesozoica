...
class CoverFlowCarouselState extends State<CoverFlowCarousel> {
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    ...
    final atTop =
        _reportedPage == 0 && position.pixels <= position.minScrollExtent + 0.5;
    ...
    if (_allowsOverscrollPull &&
      ...
      final nextPull = (_pullExtent + details.delta.dy).clamp(0.0, 280.0);
  ...
  int _targetPageForFling({required double page, required double velocityDy}) {
    ...
    final steps = (velocityDy.abs() / _flingVelocityPerPage).round().clamp(
      1,
      _maxFlingPages,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final end = math.min(widget.itemCount - 1, focused + _visibleRadius);
        final indices = [for (var i = start; i <= end; i++) i];
        final paintOrder = [...indices]
          ..sort((a, b) => (b - focused).abs().compareTo((a - focused).abs()));
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTap: widget.isFocused
          ? null
          : () {
              final carousel = context
                  .findAncestorStateOfType<CoverFlowCarouselState>();
