...
/// Active sessions always; past only when [showPastAerialRoutes] and within 24h.
List<ToolSession> aerialSessionsForMap({
    ...
    for (final session in sessions)
      if (session.isActive)
        session
          ...
          session.isPast &&
          _pastAerialRouteIsRecent(session, clock))
        session,
  ];
}
...
/// Mapbox polylines + scout pucks for aerial recon sessions.
class MapboxAerialAnnotations {
  PolylineAnnotationManager? _lineManager;
  PointAnnotationManager? _scoutManager;
  Cancelable? _tapCancelable;
  void Function(int sessionId)? _onScoutTap;
  Uint8List? _cachedPuckPng;
  String? _cachedPuckKey;
  String? _lastRouteSignature;
  final Map<int, PointAnnotation> _scoutsBySessionId = {};
  int _syncSeq = 0;
  bool _scoutUpdateInFlight = false;
  bool _scoutUpdateQueued = false;
  AerialSessionController? _pendingScoutController;

  Future<void> attach({
    required PolylineAnnotationManager lineManager,
    required PointAnnotationManager scoutManager,
    void Function(int sessionId)? onScoutTap,
  }) async {
  ...
  void _handleScoutTap(PointAnnotation annotation) {
    ...
    final sessionId = _sessionIdFromCustomData(annotation.customData);
    if (sessionId == null) return;
    onTap(sessionId);
  }

  Future<void> sync(
    AerialSessionController controller, {
    required bool showPastAerialRoutes,
  }) async {
    ...
    final routeSignature =
        '${showPastAerialRoutes ? 1 : 0}|${_routeSignature(visible)}';
    ...
    if (routeSignature != _lastRouteSignature) {
      ...
      for (final session in visible) {
        if (session.route.length < 2) continue;
  ...
  Future<void> _syncScouts({
    required AerialSessionController controller,
    required PointAnnotationManager scoutManager,
    ...
    try {
      do {
        ...
        for (final session in controller.sessions) {
          if (!(session.isInFlight || session.isPending)) continue;
          final pos = controller.scoutPosition(session, now: now);
          if (pos == null) continue;
          final bearing = controller.scoutBearing(session, now: now);
          activeIds.add(session.sessionId);

          final existing = _scoutsBySessionId[session.sessionId];
          if (existing != null) {
            ...
            updates.add(() async {
              final created = await scoutManager.create(
                PointAnnotationOptions(
                  ...
                  symbolSortKey: 10,
                  customData: {'sessionId': '${session.sessionId}'},
                ),
              );
              if (seq != _syncSeq) return;
              _scoutsBySessionId[session.sessionId] = created;
            }());
          }
        }

        for (final id in _scoutsBySessionId.keys.toList()) {
          if (activeIds.contains(id)) continue;
          final stale = _scoutsBySessionId.remove(id);
          if (stale != null) {
  ...
  String _routeSignature(List<ToolSession> sessions) {
    final buf = StringBuffer();
    for (final session in sessions) {
      buf
        ..write(session.sessionId)
        ..write(':')
        ..write(session.actionKey)
        ..write(':')
        ..write(session.status)
        ..write(':')
        ..write(session.route.length)
        ..write(';');
    }
    return buf.toString();
  }

  static int? _sessionIdFromCustomData(Map<String, dynamic>? data) {
    if (data == null) return null;
    final raw = data['sessionId'];
