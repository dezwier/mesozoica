...
/// Active timed Ridge Glass session (global visibility / discovery buffs).
class RidgeGlassController extends ChangeNotifier {
  RidgeGlassController({ToolService? toolService})
  ...
  Future<void> restoreActiveSession() async {
    ...
    try {
      ...
      _tool = null;
      _ensureTickTimer();
      _message = '${RidgeGlassKind.toolName} active';
      notifyListeners();
    } catch (error) {
      debugPrint('Ridge glass restore failed: $error');
    }
  }
  ...
  Future<void> activate(ToolSummary tool) async {
    ...
    if (!RidgeGlassKind.matchesToolName(tool.name)) return;
