...
class FractalTreeLayout {
  ...
  FractalLayoutNode _layoutNode({
    ...
    final branchLength = isRoot ? 0.0 : _branchLengthForDepth(depth);
...
class FractalLodPolicy {
  ...
  static double nodeZoomBoost(double zoomScale) {
    return (1.0 + math.log(zoomScale.clamp(1.0, 200)) / math.ln10 * 0.18).clamp(
      1.0,
      1.5,
    );
  }
  ...
  static Size genusCardScreenSize(double viewportWidth, double zoomScale) {
    final catalogWidth = viewportWidth
        .clamp(genusCardMinFaceWidth, 2000.0)
        .toDouble();
    ...
    final faceWidth = math.max(
      minTreeFaceWidth,
      treeWidth - genusCardHorizontalPadding,
    );
