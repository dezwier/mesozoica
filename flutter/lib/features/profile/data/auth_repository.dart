...
import 'package:mesozoica/core/networking/token_storage.dart';

class AuthService {
  AuthService({ApiTransport? transport, AuthTokenStore? tokenStore})
  ...
  Future<Map<String, dynamic>> register({
    ...
    try {
      ...
      final update = await _transport.patch(
  ...
  Future<Map<String, dynamic>> uploadProfileImage(List<int> bytes) async {
    try {
      final response = await _transport.multipart(
  ...
  Future<List<String>> getLinkedAccounts() async {
    final response = await _transport.get('/api/v1/auth/linked-accounts');
  ...
  Future<Map<String, dynamic>> linkGoogle({
    ...
    try {
      ...
      if (firebaseToken == null || firebaseToken.isEmpty) {
        return {
          'success': false,
          'message': 'Could not refresh Firebase token.',
        };
      }
  ...
  Future<Map<String, dynamic>> unlinkProvider(String provider) async {
    try {
      final response = await _transport.delete('/api/v1/auth/link/$provider');
      return {
        'success': true,
        'providers': (response['providers'] as List?)?.cast<String>() ?? [],
  ...
  Future<Map<String, dynamic>> deleteAccount() async {
    try {
      await _transport.delete('/api/v1/auth/delete-account');
  ...
  Future<Profile> refreshProfile() async {
    final response = await _transport.get('/api/v1/users/me');
