...
import 'package:mesozoica/controllers/guidance_session_controller.dart';
import 'package:mesozoica/controllers/main_param_buff_controller.dart';
...
import 'package:mesozoica/widgets/map/mapbox_formation_map_overlay.dart';
import 'package:mesozoica/widgets/map/mapbox_orbit_survey_overlay.dart';
import 'package:mesozoica/widgets/map/mapbox_site_annotations.dart';
import 'package:mesozoica/widgets/map/mapbox_viewport_native.dart';
import 'package:mesozoica/widgets/map/orbit_survey_raster.dart';

import 'mapbox_annotation_bundle.dart';
import 'mapbox_map_surface.dart';
...
class _MapboxFieldMapState extends State<MapboxFieldMap>
  ...
  late final MapboxAnnotationBundle _annotationBundle;
  MapboxSiteAnnotations? get _annotations => _annotationBundle.site;
  MapboxAerialAnnotations? get _aerialReconAnnotations =>
      _annotationBundle.aerial;
  final MapboxOrbitSurveyOverlay _orbitSurveyOverlay =
      MapboxOrbitSurveyOverlay();
  final MapboxFormationMapOverlay _formationMapOverlay =
      MapboxFormationMapOverlay();
  MapboxMap? _map;
  ...
  LatLng? get _effectiveLocation => _liveLocation ?? widget.currentLocation;

  @override
  void initState() {
    super.initState();
    _annotationBundle = MapboxAnnotationBundle(
    ...
    widget.aerialRecon?.progressTickListenable.addListener(
      _onAerialProgressTick,
    );
  ...
  void dispose() {
    ...
    widget.aerialRecon?.progressTickListenable.removeListener(
      _onAerialProgressTick,
    );
  ...
  void _onLocationListenable() {
    ...
    if (shouldFollow && loc != null) {
      unawaited(widget.camera.followLocation(loc, followUser: true));
    }
  ...
  bool get _pinOverlayActive => widget.rotateWithHeading || _detailPinsActive;
  ...
  void didUpdateWidget(covariant MapboxFieldMap oldWidget) {
    ...
    if (shouldFollow &&
      ...
      unawaited(
        widget.camera.followLocation(_effectiveLocation!, followUser: true),
      );
    }
    ...
    if (oldWidget.aerialRecon != widget.aerialRecon) {
      ...
      oldWidget.aerialRecon?.progressTickListenable.removeListener(
        _onAerialProgressTick,
      );
      widget.aerialRecon?.addListener(_onAerialReconChanged);
      widget.aerialRecon?.progressTickListenable.addListener(
  ...
  Future<void> _onMapCreated(MapboxMap map) async {
    ...
    try {
      await _annotationBundle.attach(map);
  ...
  Future<void> _applyBasemapLook({bool force = false}) async {
    final map = _map;
    if (map == null || !_styleLoaded) return;
    final preset = MapboxBasemapConfig.lightPresetForBrightness(
      widget.brightness,
    );
  ...
  void _onSurfaceLayout(double width, double height) {
    ...
    if (sizeChanged) _viewportSize = size;
    if (previous != height) {
      _layoutHeight = height;
      if (previous != null && (previous - height).abs() >= 1) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _refreshFollowPuckPadding();
        });
      }
    }
    if (sizeChanged &&
        ...
        _ready) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _markRotateOverlayDirty();
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return MapboxMapSurface(
      tokenReady: _tokenReady,
      ready: _ready,
      mapActive: widget.mapActive,
      rotateWithHeading: widget.rotateWithHeading,
      followUser: widget.followUser,
      detailPinsActive: _detailPinsActive,
      ridgeGlassPulseActive: _ridgeGlassPulseActive,
      camera: widget.camera,
      viewport: _viewport,
      lastKnownZoom: _lastKnownZoom,
      visibleRotateSites: _visibleRotateSites,
      selectedSite: widget.selectedSite,
      disguisedSiteId: widget.disguisedSiteId,
      onLayout: _onSurfaceLayout,
      ...
      onMapLoadError: (message) => widget.onError?.call(message),
      onRotateSiteTap: _onRotateMiniCardTap,
      onRotatePinchZoomOut: widget.onRotatePinchZoomOut,
      baseVisibilityM: _resolveVisibilityDistanceM(ignoreActiveTool: true),
