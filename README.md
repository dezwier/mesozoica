...
**Shared game config:** YAML under [`backend/app/game_config/`](backend/app/game_config/) is the game-mechanics control board and single source of truth. The backend serves the validated config at `GET /api/v1/game-config`; the Flutter client fetches it at startup (falling back to an on-device cache, then a bundled canonical snapshot generated from the backend, then the YAML) so tuning ships without an app release. The bundled snapshot (`flutter/assets/game_config.json`) is generated by `scripts/export_bundled_game_config` and drift-checked in CI. See [`docs/game-config.md`](docs/game-config.md) for the delivery architecture and the roadmap toward a DB-backed, live-editable source (incl. the admin web tool).
