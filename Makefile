...
.PHONY: help architecture-check architecture-report docs-check backend-runtime-check backend-install backend-test run-backend flutter-analyze flutter-test run-flutter test-all quality run-cron run-game-config-seed run-field-ensure-worker fetch-coordinate-masks upload-coordinate-masks-railway run-field-site-coordinate-prune run-weather-sync run-dinosaur-wiki-sync run-dinosaur-llm-enrich run-dinosaur-knowledge run-fossil-pbdb-sync run-fossil-llm-enrich run-site-sync run-site-type-sync run-tool-sync run-dinosaur-image-generate run-fossil-image-generate run-site-type-image-generate run-tool-image-generate run-tool-image-generate-local sync-dinosaur-images sync-fossil-images sync-site-type-images sync-tool-images sync-bundled-version-meta rename-site-type-images migrate-image-versions-to-v1 migrate-named-image-versions backfill-user-levels
...
help:
	...
	@echo "  run-dinosaur-knowledge       acquire + index dinosaur knowledge (all dinos)"
	...
	@echo "  make run-dinosaur-knowledge CRON_EXTRA='--dinos Tyrannosaurus --sources wikipedia openalex'"
...
run-dinosaur-knowledge:
	cd backend && RAILWAY_RUN=1 railway run $(RAILWAY_SERVICE_FLAG) python -m app.crons.runner --job dinosaur_knowledge $(CRON_EXTRA)
